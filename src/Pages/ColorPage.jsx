...
const ColorPage = () => {
    ...
    const updateColorMutation = useMutation({
        mutationFn: ({ id, newData }) => updateColor(id, newData),
        onSuccess: () => {
    ...
    return (
        <div className="w-full p-3 md:p-6 h-full overflow-auto max-h-[87vh] scrollbar-hide flex flex-col bg-base-100 text-base-content">
            ...
            <div className="flex justify-between mb-4">
                <h1 className="text-2xl sm:text-3xl font-bold">Color Management</h1>
                ...
                <table className="min-w-full table-fixed">
                    ...
                    <tbody>
                        {colors?.data?.map((color, idx) => (
                            <tr key={color._id} className="border-t border-base-content hover:bg-base-300">
                                ...
                                <td className="p-4 text-center">
                                    <button
                                        ...
                                        className={`cursor-pointer relative inline-flex h-6 w-12 items-center rounded-full transition ${color.active ? "bg-base-content" : "bg-neutral-content opacity-70"}`}
                                    >
                                        <span
                                            className={`inline-block h-5 w-5 transform rounded-full bg-base-100 transition ${color.active ? "translate-x-6" : "translate-x-1 "}`}
                                        />
                                    </button>
                                </td>
                                ...
                                <td className="p-4">
                                    <div className="flex gap-3 justify-center">
                                        <button onClick={() => startEdit(color)} className="btn btn-outline btn-primary btn-sm">
                                            Edit
                                        </button>

                                        <button
                                            onClick={() => setOpenDeleteModal({ name: color.name, id: color._id })}
            ...
            <div className="mt-auto py-2 flex justify-center items-center gap-5 sticky bottom-0 bg-base-100 bg-opacity-80 backdrop-blur-md">
                ...
                <ChevronRight
                    className="cursor-pointer hover:bg-base-300 rounded-full p-1 size-8"
                    onClick={() => page < colors?.totalPages && setPage((p) => p + 1)}
                />
            </div>
                ...
                <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50 backdrop-blur-xs p-3">
                    <div className="p-6 rounded-2xl w-full max-w-md shadow-xl border border-base-300 bg-base-200">
                        ...
                        <div className="flex justify-end gap-3">
                            ...
                            <button className="btn btn-error btn-sm text-shadow-success-content" onClick={() => { deleteColorMutation.mutate(openDeleteModal.id); setOpenDeleteModal({ name: "", id: "" }); }}>Delete</button>
                        </div>
                    </div>
                </div>
            )}
